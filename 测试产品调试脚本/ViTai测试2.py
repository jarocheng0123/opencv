...
class CameraConfig:
    ...
    READ_PARAMS = {
        ...
        if info["chinese_name"] in ["亮度调节", "白平衡色温","自动白平衡模式","自动曝光模式"]
    }
...
def print_unsupported_param_info():
    ...
    print("帧率:       硬件不支持或驱动无接口") 
    print("降噪强度:   硬件无功能或驱动不支持")
    print("数字变焦:   需光学镜头或硬件不支持")
    print("色彩模式:   硬件不支持或驱动无接口")
    print("分辨率宽度: 硬件不支持或驱动无接口")
    print("分辨率高度: 硬件不支持或驱动无接口")
...
def silent_install_deps():
    ...
    for pkg_info in required: # 遍历需要安装的包
        pkg_name, mod_name = pkg_info
        try:
            importlib.import_module(mod_name) # 尝试导入模块
        except ImportError:
            installed = False
            ...
            try:
                subprocess.run(cmd, check=True) # 执行安装命令
...
def get_camera_hardware_info():
    ...
    # 设备列表
    devices = []
    for item in c.Win32_PnPEntity(ClassGuid="{ca3e7ab9-b4c3-4ae6-8251-579ef933890f}"): # 遍历设备
        ...
        try:
            with winreg.OpenKey(
                ...
                serial = _get_reg_value(key, "SerialNumber") # 序列号
                manufacturer = _get_reg_value(key, "Manufacturer") # 制造商
        except:
...
def generate_system_report(): 
    ...
    if hw_info:
        ...
        for idx, name, vid, pid, serial, manufacturer in target_devices:
            serial = generate_sn(NAME_GF225, pid)  # 生成SN
            report.append(
        ...
        if duplicate_devices:
            ...
            device_names_map = defaultdict(list) # 重复设备名称映射表
...
def print_final_device_status(target_cameras):
    ...
    for idx, cam in enumerate(target_cameras, 1): # 打印摄像头状态
        status = "✅" if not cam.failed_params else "❌"
        failed_param_names = [cam._get_ch_name(param) for param in cam.failed_params] # 失败参数名称列表
        failed_param_str = ", ".join(failed_param_names) # 失败参数名称
        if failed_param_str:
            failed_param_str = f"({failed_param_str})"
        print(f"摄像头{idx}: {status} ViTai-{cam.vid}-{cam.pid}{failed_param_str}") # 打印摄像头状态
...
class Camera:
    ...
    def apply_parameters(self):
        ...
        for param_name, cv_constant in self.PARAM_MAP.items(): # 遍历所有参数
            ...
            try:
                success = self.cap.set(cv_constant, float(clamped_value)) # 设置参数
                if success:
                    print(
    ...
    def show_video_stream(self):
        if SHOW_CAMERA_WINDOW:
            try:
                window_name = f"ViTai-{self.vid}-{self.pid}" # 窗口名称
                cv2.namedWindow(window_name, cv2.WINDOW_NORMAL) # 创建窗口
                cv2.resizeWindow(window_name, 640, 480) # 设置窗口大小

                while not exit_event.is_set(): # 全局作用域定义退出事件
                    ...
                    if ret:
                        ...
                        cv2.imshow(window_name, frame) # 显示视频流
                        cv2.waitKey(1) # 等待1毫秒
    ...
    def get_param_value(self, param_const):
        if self.cap and self.cap.isOpened(): # 检查摄像头是否打开
            return self.cap.get(param_const) # 获取参数值
        return None
...
# 关闭摄像头属性窗口函数
def close_camera_property_windows():
    max_retries = 3  # 最大重试次数
    for _ in range(max_retries): 
        def enum_windows_callback(hwnd, lparam):
            window_text = win32gui.GetWindowText(hwnd).lower() # 获取窗口文本
            for keyword in KEYWORDS: 
                if keyword.lower() in window_text: # 匹配关键字
...
def main():
    ...
    try:
        ...
        for cam in target_cameras: # 释放摄像头资源
            cam.cleanup()
        
        if SHOW_PROPERTY_WINDOW:
            print(f"\n{RED}关闭{KEYWORDS}窗口{RESET}") 
            close_camera_property_windows() # 关闭属性窗口
